...
class SiteUnreadMsg(_PluginBase):
    ...
    def get_page(self) -> List[dict]:
        ...
        # 最近两天的未读消息数据
        unread_data: Dict[str, Dict[str, Any]] = {}
        for day in date_list:
            current_day = day.strftime("%Y-%m-%d")
            unread_data = self.get_data(current_day)
            if unread_data:
                break
        if not unread_data:
            return [
        ...
        # 数据按时间降序排序
        unread_data = dict(sorted(unread_data.items(),
        ...
        unread_msgs = [
            ...
            } for site, data in unread_data.items() if not data.get("err_msg")
        ]
